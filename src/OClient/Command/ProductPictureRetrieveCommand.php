...
class ProductPictureRetrieveCommand extends Command {
    ...
    protected function configure() {
        ...
        $this->setName('product:picture:retrieve')
                ...
                ->addArgument(
                        'path', InputArgument::REQUIRED, 'What is the path where to save the pictures ?'
                )
                ->addOption(
                        'layout', null, InputOption::VALUE_REQUIRED, 'Layout to use for generating directory structure'
                )
                ->addOption(
                        'resolution', null, InputOption::VALUE_REQUIRED, 'Resolution of the pictures i.e: 800x800'
                )
                ->addOption(
                ...
                ->addOption(
                        'overwrite', null, InputOption::VALUE_NONE, 'Overwrite existing picture files.'
                )
    ...
    protected function getOptions(InputInterface $input, OutputInterface $output) {
        ...
        if ($layout == '') {
            $idx = $dialog->select(
                    $output, "Please select the directory layout (default to $default_layout)", $supported_layouts, $default_layout
            );
        ...
        if ($resolution == '') {
            $resolution = $dialog->select(
                    $output, "Please select the resolution of the pictures (default to $default_resolution)", $supported_resolutions, $default_resolution
            );
        ...
        if ($quality == '') {
            $quality = $dialog->select(
                    $output, "Please select the quality of the pictures (default to $default_quality)", $supported_qualities, $default_quality
            );
